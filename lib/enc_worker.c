...
struct oc_enc_wmb_data {
  ...
  unsigned i0mv_satd[12];
  unsigned g0mv_satd[12];

  enum oc_enc_wrk_computed computed;
  unsigned i1mv_satd[12];
  unsigned g1mv_satd[12];
  unsigned i1mvr_satd[12];
  unsigned g1mvr_satd[12];
};
...
static void inter_analysis(oc_enc_ctx *_enc,int _recode,unsigned mbi,struct oc_enc_wmb_data *d) {
  ...
  if(_enc->state.sb_flags[sbi].quad_valid&1<<quadi){
    unsigned activity[4];
    unsigned luma;
    luma=oc_mb_intra_satd(_enc,mbi,d->intra_satd);
    /*Activity masking.*/
    if(sp_level<OC_SP_LEVEL_FAST_ANALYSIS)
      oc_mb_activity(_enc,mbi,activity);
    else
      oc_mb_activity_fast(_enc,mbi,activity,d->intra_satd);
    ...
    oc_cost_inter_nomv_satd(_enc,mbi,OC_MODE_INTER_NOMV,d->i0mv_satd);
    oc_cost_inter_nomv_satd(_enc,mbi,OC_MODE_GOLDEN_NOMV,d->g0mv_satd);
    d->computed = 0;
    ...
    if(sp_level<OC_SP_LEVEL_NOMC){
      ...
      oc_cost_inter1mv_satd(_enc,mbi,OC_MODE_INTER_MV,
                            embs[mbi].unref_mv[OC_FRAME_PREV],d->i1mv_satd);
      oc_cost_inter1mv_satd(_enc,mbi,OC_MODE_INTER_MV,
                            embs[mbi].analysis_mv[0][OC_FRAME_PREV],d->i1mvr_satd);
      oc_cost_inter1mv_satd(_enc,mbi,OC_MODE_GOLDEN_MV,
                            embs[mbi].unref_mv[OC_FRAME_GOLD],d->g1mv_satd);
      ...
      oc_cost_inter1mv_satd(_enc,mbi,OC_MODE_GOLDEN_MV,
                            embs[mbi].analysis_mv[0][OC_FRAME_GOLD],d->g1mvr_satd);
...
#endif
  unsigned current_capacity = w->mbi_end - w->mbi_start + 1;
  w->mbi_start = sbi_start << 2;
  w->mbi_end = (sbi_end << 2) - 1;
  unsigned blocks = w->mbi_end - w->mbi_start + 1;
  w->recode = !!recode;
  w->intra = !!intra;
  w->stall = 1;
...
void oc_enc_worker_get_nomv_satd(oc_enc_ctx *enc, unsigned mbi, unsigned **skip_ssd, unsigned **intra_satd,
  ...
  *intra_satd = w->mbs[wmbi].intra_satd;
  *inter_satd = w->mbs[wmbi].i0mv_satd;
  *golden_satd = w->mbs[wmbi].g0mv_satd;
}

void oc_enc_worker_get_mv_satd(oc_enc_ctx *enc, unsigned mbi, int which, int refined, unsigned **satd) {
  struct oc_enc_worker_ctrl *w = enc->threads;
  unsigned wmbi = w->workers ? mbi - w->mbi_start : 0;
  ...
  switch(which) {
    case OC_MODE_INTER_MV:
      ...
      *satd = refined ? d->i1mvr_satd : d->i1mv_satd;
      ref_frame = OC_FRAME_PREV;
      break;
    case OC_MODE_GOLDEN_MV:
      computed = refined ? WRK_G1MV_REFINED : WRK_G1MV_EST;
      *satd = refined ? d->g1mvr_satd : d->g1mv_satd;
  ...
  if(!(d->computed & computed)) {
    ...
    oc_cost_inter1mv_satd(enc,mbi,which,mvs[ref_frame],*satd);
    d->computed |= computed;
  }
}
